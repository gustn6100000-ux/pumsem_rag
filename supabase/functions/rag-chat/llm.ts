...
3. **표 형식 — 컨텍스트 구조 유지**: 컨텍스트에 제공된 4열 플랫 구조 표(직종/장비명/자재명 | 수량 | 단위 | 규격)를 **있는 그대로** 유지하여 출력합니다.
   - 표 구조는 제공된 데이터를 기준으로 작성하며, 기준 당 하나의 테이블로 병합하지 않고 행으로 개별 표시합니다.
...
| 특별인부 | 0.086 | 인 | 200, SCH 20 |

※ 표의 단위 정보 등에 포함된 값을 그대로 표기합니다.
...
// ─── Phase 2: 복합 추론 가이드 생성 (DeepSeek Reasoner) ───
export async function generateReasoningGuide(
    question: string,
    history: ChatMessage[]
): Promise<{ search_tasks: string[], calculations: string[], adjustments: string[] } | null> {
    if (!DEEPSEEK_API_KEY) return null;

    const systemInstruction = `당신은 건설 품셈 시스템의 작업 분석가입니다.
사용자의 질문이 여러 공종이 혼합되거나 복잡한 조건(할증, 다수 규격 등)을 포함하고 있습니다.
검색 시스템이 어떤 한글 키워드들로 품셈을 검색해야 할지, 계산 로직은 어떤 순서로 해야 할지 마스터플랜을 작성하세요.

반드시 다음 JSON 형식만 출력하세요. 마크다운(\`\`\`json)으로 감싸지 마세요.
{
  ...
  "adjustments": ["고소작업 20% 할증 적용 여부 확인"]
    ...
    // deepseek-reasoner는 roles=["user", "assistant"] 조합만 강력 권장하므로 system을 user 메시지 맨 앞에 합성
    const messages = [
    ...
    try {
        ...
        const response = await fetch(DEEPSEEK_URL, {
            ...
            body: JSON.stringify({
                ...
                messages,
                // temperature 속성은 reasoner에서 제거 권장
            }),
        ...
        if (response.ok) {
            ...
            console.error(`[generateReasoningGuide] Reasoner HTTP Error: ${response.status}`);
        }
    } catch (err) {
