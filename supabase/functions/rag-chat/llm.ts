...
3. **표 형식 — 컨텍스트 구조 유지**: 컨텍스트에 제공된 테이블 구조를 **있는 그대로** 유지하여 출력합니다.
   ...
   - **다중 항목 교차표**: 용접봉소비량·소요전력·인력 등 여러 항목이 하나의 교차표에 포함되어 있으면, 반드시 **하나의 통합 테이블**로 출력합니다. 카테고리별로 나누어 별도의 소테이블로 분리하지 않습니다.
...
📋 **[표 13-2-4] 강판 전기아크용접 — H형** (m당)

| 두께(mm) | 용접봉_한면 | 용접봉_양면 | 전력_한면 | 전력_양면 | 인력_한면_용접공 | 인력_한면_특별인부 | 인력_양면_용접공 | 인력_양면_특별인부 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 15 | 1.60 | 1.70 | 4 | 8 | 0.114 | 0.034 | 0.165 | 0.050 |

※ 위처럼 용접봉·소요전력·인력을 **하나의 행**에 모두 포함하여 한눈에 비교할 수 있게 합니다.
※ 절대 카테고리별(용접봉 테이블, 소요전력 테이블, 인력 테이블)로 세로 분할하지 않습니다.` + `
