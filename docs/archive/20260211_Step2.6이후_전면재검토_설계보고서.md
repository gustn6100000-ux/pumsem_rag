...
CREATE POLICY "anon_read_global_rel" ON graph_global_relationships FOR SELECT TO anon USING (true);
...
- `graph_global_relationships`: `normalized_entities.json` → `global_relationships`에서 `HAS_CHILD`(1,061건), `REFERENCES`(2건) 별도 적재
